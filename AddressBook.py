...
class AddressBook(UserList):
    ...
    def log(self, action):
        with open("log.txt", 'a') as log:
            log.write(dt.now().strftime("%Y-%m-%d %H:%M:%S") + " - " + action + '\n')
...
def main():
    ...
    while True:
        ...
        if command == "add":
            ...
            phones = ui.get_input("Enter phones (separated by comma): ").split(",")
            ...
            category = ui.get_input("Enter search category (name, email, status, note): ")

            address_book.search(pattern, category)

        elif command == "edit":
            contact_name = ui.get_input("Enter the name of the contact to edit: ")
            parameter = ui.get_input("Enter the parameter to edit (name, phones, birthday, email, status, note): ")
